...
const signUp = async (req, res) => {
  ...
  try {
    ...
    connection.query(query, [name, email, hashedPassword, role || "user"], 
      (err, results) => {
      if (err) {
        if (err.code === "ER_DUP_ENTRY") {
          return res.status(400).json(
            { status: "fail", message: "L'utilisateur existe déjà." });
        }
...
const signIn = async (req, res) => {
  ...
  if (!email || !password) {
    return res.status(400).json(
      { status: "fail", message: "Tous les champs sont requis." });
  }
    ...
    connection.query(query, [email], async (err, results) => {
      ...
      if (results.length === 0) {
        return res.status(401).json(
          { status: "fail", message: "Identifiants invalides." });
      }
